...
export default function EventDetailsPage({ params }: PageProps) {
  ...
  const handleApplePayClick = async () => {
    ...
    if (!buyerName.trim()) {
      setRegistrationError("Veuillez saisir votre nom complet avant de procéder au paiement avec Apple Pay.");
      return;
    }
    ...
    if (!ApplePaySession) {
      setRegistrationError("Apple Pay n'est pas pris en charge sur ce navigateur. Ouvrez le site dans Safari sur iOS/Mac.");
      return;
    }

    try {
      const totalPrice = (ticketCount * selectedTier.price).toFixed(2);
      const paymentRequest = {
        countryCode: 'FR',
        currencyCode: 'EUR',
        supportedNetworks: ['visa', 'masterCard', 'cartesBancaires', 'amex'],
        merchantCapabilities: ['supports3DS'],
        total: {
          label: `FMB Experience - ${selectedTier.label}`,
          amount: totalPrice
        }
      };
      ...
      session.onpaymentauthorized = async (evt: any) => {
        setRegistering(true);
        try {
          const partsToSend = event!.category === 'trip' && selectedTier.capacity && selectedTier.capacity > 1 
            ? participants.map(p => p.trim()) 
          ...
          if (result.success && result.confirmationCode) {
            ...
            setRegistrationError(result.error || "Paiement réussi mais échec de l'enregistrement.");
          }
        } catch (err) {
          session.completePayment(ApplePaySession.STATUS_FAILURE);
          setRegistrationError("Erreur lors de la validation du paiement Apple Pay.");
        } finally {
          setRegistering(false);
        }
      };
      ...
      session.oncancel = () => {
        console.log('Session Apple Pay annulée');
      };

      session.begin();
    } catch (err) {
      console.error("Apple Pay Exception:", err);
      setRegistrationError("Impossible de lancer la session Apple Pay.");
    }
  };
                  ...
                  <div className="space-y-3 pt-2">
                    ...
                    {(event?.payment_mode === 'wire_transfer' || (event?.payment_mode !== 'paypal' && activePaymentTab === 'wire')) ? (
                      ...
                      <div className="pt-2 space-y-3">
                        {/* Dedicated Native Apple Pay Button */}
                        <button
                          type="button"
                          disabled={registering}
                          onClick={handleApplePayClick}
                          className="w-full py-3.5 bg-black hover:bg-zinc-900 text-white rounded-xl font-black text-sm uppercase tracking-wider transition-all flex items-center justify-center gap-2 border border-white/20 shadow-[0_0_20px_rgba(255,255,255,0.15)] hover:border-white/40"
                        >
                          ...
                          {registering ? 'Paiement en cours...' : 'Payer avec Apple Pay'}
                        </button>

                        <div className="relative flex py-1 items-center">
                          <div className="flex-grow border-t border-white/10"></div>
                          <span className="flex-shrink mx-3 text-[10px] text-white/30 font-bold uppercase tracking-widest">ou avec PayPal / CB</span>
                          <div className="flex-grow border-t border-white/10"></div>
                        </div>
